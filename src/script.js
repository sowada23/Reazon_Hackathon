...
// --- ボスデータ (会社員テーマ版 5連戦) ---
const bossStages = [
    { name: '1F：怒りの部長', maxHp: 100, color: '#e43b44', type: 'manager', maxWaves: 2, interval: 90, wayCount: 3, restDuration: 160 },
    { name: '2F：ポンコツなITコンサルタント', maxHp: 140, color: '#0099db', type: 'consultant', maxWaves: 3, interval: 80, wayCount: 4, restDuration: 140 },
    { name: '3F：ムチャ振りな取締役', maxHp: 180, color: '#f77622', type: 'director', maxWaves: 4, interval: 70, wayCount: 5, restDuration: 120 },
    { name: '4F：鬼のクライアント', maxHp: 220, color: '#fee761', type: 'client', maxWaves: 5, interval: 62, wayCount: 6, restDuration: 100 },
    { name: '5F：深夜残業の悪夢', maxHp: 260, color: '#b55088', type: 'nightmare', maxWaves: 6, interval: 52, wayCount: 8, restDuration: 90 }
];
...
// 【要求仕様】横から見た一般の会社員（1マス/数フレームごとに右足・左足を交互にアニメーション）
function drawPlayer(ctx, x, y, w, h) {
    ...
    // 6フレームごとに足を反転させて交互に走っているように見せる
    let legFlip = Math.floor(gameFrame / 6) % 2 === 0;

    // 1. 体（紺色のビジネススーツ）
    ctx.fillStyle = '#222533'; 
    ctx.fillRect(x + 10, y + 24, w - 20, h - 38);

    // 2. 頭・肌（ピーチベージュ）
    ctx.fillStyle = '#f5bca9'; 
    ctx.fillRect(x + 14, y + 6, w - 28, 18);
    
    // 3. 髪の毛（茶色）
    ctx.fillStyle = '#5c4033'; 
    ctx.fillRect(x + 12, y, w - 22, 8);
    ctx.fillRect(x + 28, y + 6, 10, 6); // もみあげ

    // 4. 赤いネクタイと白いシャツ
    ctx.fillStyle = '#ffffff';
    ctx.fillRect(x + 22, y + 24, 6, 8);
    ctx.fillStyle = '#ac3232'; // ネクタイ赤
    ctx.fillRect(x + 24, y + 26, 3, 12);

    // 5. カバン（ビジネスバッグを手に持っているドット）
    ctx.fillStyle = '#663931';
    ctx.fillRect(x + 2, y + 36, 12, 10);

    // 6. 【要求仕様】足のアニメーション（右足・左足を交互に前後に動かす）
    ctx.fillStyle = '#1c1c24'; // 靴・ズボンの暗い色
    if (legFlip) {
        // パターンA: 右足が前、左足が後ろ
        ctx.fillRect(x + 8, y + 50, 8, 14);  // 左足（後ろ）
        ctx.fillRect(x + 24, y + 50, 10, 14); // 右足（前）
        // 靴のつま先（右向きに走っている）
        ctx.fillRect(x + 32, y + 60, 4, 4);
    } else {
        // パターンB: 左足が前、右足が後ろ
        ctx.fillRect(x + 12, y + 50, 10, 14); // 左足（前）
        ctx.fillRect(x + 26, y + 50, 8, 14);  // 右足（後ろ）
        // 靴のつま先
        ctx.fillRect(x + 20, y + 60, 4, 4);
    }
...
function drawBoss(ctx, b, type) {
    ctx.save();
    if (type === 'manager') {
        // 1: 怒りの部長 (赤い怒り顔、ピクセルグレーのスーツ)
        ctx.fillStyle = '#5a5f67'; ctx.fillRect(b.x + 20, b.y + 40, b.w - 40, b.h - 40); // スーツ
        ctx.fillStyle = '#e43b44'; ctx.fillRect(b.x + 30, b.y + 10, b.w - 60, 30);      // 怒り赤顔
        ctx.fillStyle = '#000000'; ctx.fillRect(b.x + 40, b.y + 18, 8, 4); ctx.fillRect(b.x + b.w - 48, b.y + 18, 8, 4); // 怒り目
    } 
    else if (type === 'consultant') {
        // 2: ITコンサルタント (スタイリッシュな青スーツ、インテリ眼鏡)
        ctx.fillStyle = '#1b3a4b'; ctx.fillRect(b.x + 25, b.y + 35, b.w - 50, b.h - 35);
        ctx.fillStyle = '#fdd2b5'; ctx.fillRect(b.x + b.w/2 - 20, b.y + 5, 40, 30);
        ctx.fillStyle = '#ffffff'; ctx.fillRect(b.x + b.w/2 - 16, b.y + 14, 10, 6); ctx.fillRect(b.x + b.w/2 + 6, b.y + 14, 10, 6); // 眼鏡
        ctx.strokeStyle = '#333'; ctx.lineWidth = 2; ctx.strokeRect(b.x + b.w/2 - 16, b.y + 14, 10, 6); ctx.strokeRect(b.x + b.w/2 + 6, b.y + 14, 10, 6);
    } 
    else if (type === 'director') {
        // 3: ムチャ振り取締役 (高級黒スーツに立派な白ひげ)
        ctx.fillStyle = '#191919'; ctx.fillRect(b.x + 20, b.y + 40, b.w - 40, b.h - 40);
        ctx.fillStyle = '#fcd0a1'; ctx.fillRect(b.x + 30, b.y + 10, b.w - 60, 30);
        ctx.fillStyle = '#dfdfdf'; ctx.fillRect(b.x + 26, b.y + 26, b.w - 52, 16); // 豊かなヒゲ
    }
    else if (type === 'client') {
        // 4: 鬼のクライアント (ゴールドネクタイ、厳しい表情)
        ctx.fillStyle = '#3d281d'; ctx.fillRect(b.x + 20, b.y + 40, b.w - 40, b.h - 40);
        ctx.fillStyle = '#fcd0a1'; ctx.fillRect(b.x + 30, b.y + 10, b.w - 60, 30);
        ctx.fillStyle = '#fee761'; ctx.fillRect(b.x + b.w/2 - 3, b.y + 40, 6, 20); // 金ネクタイ
    }
    else if (type === 'nightmare') {
        // 5: 深夜残業の悪夢 (紫のモンスター化したオフィスの闇)
        ctx.fillStyle = '#3a2752'; ctx.fillRect(b.x + 15, b.y + 15, b.w - 30, b.h - 30);
        ctx.fillStyle = '#964b00'; ctx.fillRect(b.x + 25, b.y + 50, b.w - 50, 40); // デスクが融合した姿
        ctx.fillStyle = '#ff0055'; ctx.fillRect(b.x + 35, b.y + 25, 12, 12); ctx.fillRect(b.x + b.w - 47, b.y + 25, 12, 12); // 赤く不気味に光る目
    }
    
    if (b.isResting) {
        ctx.fillStyle = '#fee761'; ctx.font = 'bold 16px Courier';
        ctx.fillText('休 憩 中', b.x + b.w/2 - 25, b.y - 10);
    }
    ctx.restore();
}
...
function drawObstacle(ctx, obs) {
    ...
    if (obs.type === 'cross') { 
        ...
        // 時計の針ドット
        ctx.fillStyle = '#e43b44'; ctx.fillRect(obs.x + obs.w/2 - 1, obs.y + 4, 3, obs.h/2 - 2);
        ctx.fillRect(obs.x + obs.w/2, obs.y + obs.h/2 - 1, obs.w/3, 3);
    }
    ctx.restore();
}
...
function drawCandy(ctx, c) {
    ctx.save();
    if (c.isHeart) {
        // 【要求仕様】ライフ回復アイテム：オフィスのコーヒーカップ☕
        ctx.fillStyle = '#dedede'; ctx.fillRect(c.x - 12, c.y - 6, 24, 20); // カップ本体
        ctx.fillStyle = '#634b35'; ctx.fillRect(c.x - 9, c.y - 4, 18, 4);   // コーヒー液面
        ctx.fillStyle = '#dedede'; ctx.fillRect(c.x + 12, c.y - 2, 6, 10);  // 取っ手
        ctx.strokeStyle = '#5a5a5a'; ctx.lineWidth = 2; ctx.strokeRect(c.x - 12, c.y - 6, 24, 20);
    } else {
        // 通常アイテム：会社の書類 (白に数行のドットテキスト)
        ...
        ctx.fillRect(c.x - 8, c.y - 8, 16, 2);
        ctx.fillRect(c.x - 8, c.y, 12, 2);
        ctx.fillRect(c.x - 8, c.y + 8, 16, 2);
    }
    ctx.restore();
}
